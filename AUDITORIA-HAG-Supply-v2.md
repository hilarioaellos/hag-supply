...
**Fecha:** 15 julio 2026  
**Versión:** MVP v2 — Correcciones post-auditoría v1  
**Stack:** Next.js 16 · TypeScript · Tailwind v4 · Prisma 5 · NextAuth 4 · Stripe · bcryptjs · Resend

---

## Cambios respecto a v1 (respuesta a los 3 bloqueantes)

### Bloqueante 1 — middleware.ts reescrito
- Eliminada variable `isProtectedRoute` (estaba computada pero nunca usada en ningún condicional)
- Separada autenticación de autorización:
  - No autenticado → `/login?redirect=...`
  - Autenticado sin rol ADMIN en ruta admin → `/403` (no `/login`)
- El matcher define el scope; el middleware solo distingue los dos casos

### Bloqueante 2 — Stack
- Baseline actualizado a Next.js 16 + Tailwind v4 (gate de arquitectura aprobado)
- El plan de implementación sigue aplicando sin cambios

### Bloqueante 3 — Tipado de `role`
- `types/next-auth.d.ts`: `role` ahora tipado como `Role` (enum de Prisma), no `string`
- `lib/auth.ts`: import de `Role` en uso; eliminados todos los casts `as string`
- Contrato fuerte DB → JWT → Session trazable end-to-end
...
export async function middleware(req: NextRequest) {
  ...
  // No autenticado → login (aplica a todas las rutas del matcher)
  if (!token) {
  ...
  // Autenticado pero sin rol ADMIN → 403 (no es falta de login, es falta de permiso)
...
> **Nota:** El índice parcial `one_pending_per_user` se agrega manualmente al SQL de migración:
> ```sql
...
export function Badge({ variant }: { variant: ProductBadge }) {
  return (
    <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${styles[variant]}`}>
      {labels[variant]}
    </span>
  );
}
  ...
  ({ label, error, className = "", id, ...props }, ref) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        {label && (
          <label htmlFor={inputId} className="text-sm font-medium text-hag-text">
            {label}
          </label>
        )}
        <input
...
export function Modal({ open, onClose, children, title }: ModalProps) {
  ...
  return (
    ...
    >
      ...
      >
        {title && <h2 className="text-lg font-bold text-hag-text mb-4">{title}</h2>}
        <button
          onClick={onClose}
      ...
      {visible.map((page, i) => {
        ...
        return (
          <span key={page} className="flex items-center gap-1">
            {gap && <span className="px-1 text-hag-text-3">&hellip;</span>}
            <button
...
const icons: Record<AlertVariant, string> = {
  info: "i", warning: "!", error: "x", success: "v",
};
...
const CATEGORIES = [
  ...
  { name: "Deals and Offers", slug: "/deals" },
];
      ...
      <div className="px-12 pt-14 pb-7">
        <div className="grid grid-cols-[1.2fr_1fr_1fr_1fr_1.3fr] gap-8 pb-10 border-b border-white/10">
          <div>
            ...
            <ul className="flex flex-col gap-[11px] text-[14px]">
              {CATEGORIES.map((c) => (
                <li key={c.slug}>
                  <Link
                    href={c.slug.startsWith("/") ? c.slug : `/category/${c.slug}`}
          ...
          <div>
            ...
            <p className="text-[13.5px] mb-3.5 leading-relaxed">
              Get weekly deals and new arrivals in your inbox.
            </p>
        ...
        <div className="flex items-center justify-between pt-6">
          <div className="flex items-center gap-3.5">
            ...
            <span className="text-[12.5px] text-white/45">
              &copy; 2026 HAG Supply &mdash; A Division of HAG Partner LLC
            </span>
          </div>
          <div className="flex gap-2.5">
            {[
              ...
              <a
                ...
                href="#"
                aria-label={s.label}
                className="w-[30px] h-[30px] rounded-full border border-white/25 flex items-center justify-center text-[12px] font-semibold hover:border-white/50 transition-colors"
              >
                {s.abbr}
              </a>
            ))}
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-hag-border">
          <h2 className="text-base font-bold text-hag-text">
            Your Cart{" "}
            {items.length > 0 && (
          ...
          <button
            onClick={closeDrawer}
            className="text-hag-text-3 hover:text-hag-text text-lg"
                ...
                <div key={item.id} className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 min-w-0">
                    <Link
                      ...
                      onClick={closeDrawer}
                      className="text-sm font-semibold text-hag-text line-clamp-2 hover:text-hag-accent"
                    >
                      {item.product.name}
                    </Link>
                  ...
                  <p className="text-sm font-bold text-hag-text whitespace-nowrap">
...
## Puntos abiertos conocidos (no bloqueantes para esta fase)

Los siguientes items fueron identificados en la auditoría v1 como mayores/medios y quedan pendientes para fases posteriores:

1. **Header + CategoryNav:** dos consultas DB separadas por render — se resolverá extrayendo `getCategories` a `lib/data/categories.ts` en HAG-9.
2. **CartProvider:** llama `/api/cart` al montar aunque el usuario no esté autenticado — se ajustará cuando se implemente la API del carrito (HAG-14).
3. **CartDrawer:** precios calculados con `parseFloat` sobre strings — se normalizará cuando se defina el contrato de serialización del API del carrito.
4. **SearchBar:** dropdown no cierra con click externo — se corregirá en HAG-9.
5. **Modal:** sin focus trap — se corregirá antes de usar en flujos de checkout/auth.
6. **next.config.ts:** sin `images.remotePatterns` — se configurará en HAG-9 cuando se definan los dominios de imágenes reales.

---

*Fin del documento — HAG Supply MVP v2*
