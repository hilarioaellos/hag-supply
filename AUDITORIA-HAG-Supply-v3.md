...
**Fecha:** 15 julio 2026
**Versión:** MVP v3 — Correcciones post-auditoría v2
**Stack:** Next.js 16.2 · TypeScript 5 · Tailwind v4 · Prisma 5.22 · NextAuth 4.24 · Stripe 22 · bcryptjs · Resend

---

## Cierre de bloqueantes v2

### Bloqueante 1 — Confirmación formal de baseline

El cambio de stack de Next.js 14 + Tailwind v3 a Next.js 16 + Tailwind v4 fue
confirmado explícitamente por el propietario del proyecto (Hilario Aellos) durante
esta sesión de construcción, al presentársele ambas opciones y elegir mantener
Next.js 16 + Tailwind v4 como nuevo baseline.

Baseline oficial desde esta versión:
- Next.js 16.2 (App Router)
- Tailwind v4 (configuración via CSS @theme)
- El resto del plan de implementación (HAG-5 a HAG-28) sigue aplicando sin cambios.

### Bloqueante 2 — Tipado fuerte en lib/auth.ts sin casts

El cast `(user as unknown as { role: Role }).role` fue eliminado completamente.

Solución: extender la interfaz `User` de NextAuth en `types/next-auth.d.ts` para
incluir `role: Role`. Con eso, `user.role` en el callback `jwt` es correctamente
tipado como `Role` desde Prisma sin ninguna coerción manual.

El contrato DB → JWT → Session ahora es fuerte y trazable end-to-end sin casts.

---

## Archivos modificados en v3

Solo 2 archivos cambiaron respecto a v2:
...
export async function middleware(req: NextRequest) {
  ...
  // Autenticado sin permiso de admin: 403
...
> Indice parcial a agregar manualmente al SQL de migracion:
> ```sql
        ...
        <div className="flex items-center justify-between pt-6">
          <div className="flex items-center gap-3.5">
            ...
            <span className="text-[12.5px] text-white/45">
              2026 HAG Supply - A Division of HAG Partner LLC
            </span>
          </div>
...
|---|-------------|----------------|
| 1 | Header + CategoryNav hacen dos consultas DB separadas por render | HAG-9 (extraccion a lib/data/categories.ts) |
| 2 | CartProvider llama /api/cart al montar aunque el usuario no este autenticado | HAG-14 (implementacion de /api/cart) |
| 3 | CartDrawer calcula precios con parseFloat sobre strings | HAG-14 (normalizacion del contrato de serializacion) |
| 4 | SearchBar: dropdown no cierra con click externo | HAG-9 |
| 5 | Modal: sin focus trap | Antes de HAG-13 (flujos de auth) |
| 6 | next.config.ts: sin images.remotePatterns | HAG-9 (cuando se definan dominios de imagenes reales) |

---

*Fin del documento - HAG Supply MVP v3*
